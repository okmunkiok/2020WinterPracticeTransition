...
//void transmitRangeReport() {
    ...
    DW1000Time deltaTime = DW1000Time(replyDelayTimeUS, DW1000Time::MICROSECONDS);
    timeRangeReportSent = DW1000.setDelay(deltaTime);
    timePollAckSent.getTimestamp(data + 1);
    timeRangeReceived.getTimestamp(data + 6);
    timeRangeReportSent.getTimestamp(data + 11);
...
void loop() {
    ...
    if (receivedAck) {
        ...
        byte msgId = data[0];
        byte anchorId = data[DATA_INDEX_BYTE_FOR_ANCHORS];
        if (anchorId != WHICH_ANCHOR_AM_I){
          return;
        }
